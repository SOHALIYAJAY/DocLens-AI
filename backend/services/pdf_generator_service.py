...
def extract_links_from_pdf_bytes(file_bytes: bytes) -> list:
    ...
    for i in range(len(doc)):
        ...
        # 1. Interactive Hyperlinks (annotations)
        try:
        ...
        # 2. Plain Text URLs (Regex search)
        try:
...
def add_footer(canvas, doc):
    ...
    canvas.drawString(72, 32, "Powered by AI PDF Assistant")
    canvas.drawRightString(540, 32, f"Page {doc.page}")
    canvas.restoreState()

def create_navigator_pdf(navigator: NavigatorResponse) -> bytes:
    buffer = io.BytesIO()
    ...
    c_bg_light = colors.HexColor('#f8fafc')  # Very light grey/blue
    ...
    for section in navigator.sections:
        ...
        for item in section.items:
            ...
            if "code" in sec_title_lower:
                ...
                # Formulas get light yellow styled background block
                ...
                container.setStyle(TableStyle([
                    ('BACKGROUND', (0,0), (-1,-1), colors.HexColor('#fef08a')), # Light yellow
                ...
                # Definitions bold the term and indent definition
                ...
                # Document links render as clickable blue links
